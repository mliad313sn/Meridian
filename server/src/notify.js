...
import { QUIET_DAYS } from "./adoption.js";
...
export async function sweep({ today = new Date().toISOString().slice(0, 10), horizonDays = 3 } = {}) {
  ...
  /* ── O-2 (docs/32 → docs/36 C-04) — the kinds the vocabulary promised
     (013, widened by 018) and nothing fed. Five emitters, same shape as
     the two above: read wide, address narrowly, dedupe by what it is
     about. ─────────────────────────────────────────────────────────── */
  ...
  /* A site's formal voice on a group project must reach the project's
     manager once, and stay in the box until read — the escalator raises
  ...
  /* A-08 — a book nobody has touched in thirty days. Addressed to the
     ...
     person told first. Same progress signal, same threshold, as the
     Adoption screen (adoption.js). */
  const quiet = await many(
    `WITH progress AS (
     ...
     HAVING g.last_at IS NULL OR g.last_at < now() - ($1::int * interval '1 day')`, [QUIET_DAYS]);
  ...
  /* R-03 — a completed week with an allocation and no recorded days.
     Once per week, to the person, never to their manager and never to a
     deputy: the entry is theirs to make, and a nag that goes over
     someone's head is a surveillance tool, not a reminder. */
  const noWeek = await many(
    `SELECT per.id AS pid, u.id AS user_id, u.email, u.locale, u.notify_pref,
            (date_trunc('week', $1::date)::date - 7)::text AS wk
       FROM person per
       JOIN app_user u ON u.person_id = per.id AND u.active
      WHERE per.active
        AND EXISTS (SELECT 1 FROM allocation al
                     WHERE al.person_id = per.id
                       AND al.from_date <= date_trunc('week', $1::date)::date - 1
                       AND al.to_date   >= date_trunc('week', $1::date)::date - 7)
        AND NOT EXISTS (SELECT 1 FROM timesheet ts
                     WHERE ts.person_id = per.id
                       AND ts.week_start = date_trunc('week', $1::date)::date - 7)`, [day]);
  for (const w of noWeek) {
    if ((w.notify_pref ?? "immediate") === "off" || !w.email) continue;
    queued += await queue({
      userId: w.user_id, email: w.email, kind: "timesheet-missing", severity: "info",
      subject: inLocale("Last week's effort is not recorded", w.locale),
      body: inLocale("You were allocated to project work last week and no days are recorded. Four fields, once a week — the real sits beside the plan, and the gap is the point.", w.locale),
  ...
  for (const u of readers) {
    const bucket = u.notify_pref === "daily" ? day : "w" + weekBucket;
    queued += await queue({
      userId: u.user_id, email: u.email, kind: "digest", severity: "info",
      subject: inLocale("Your Meridian digest is ready", u.locale),
      body: inLocale("Everything that changed in your scope, in one page: open Reports, then the digest.", u.locale),
...
export async function deliver(send, { limit = 50 } = {}) {
  ...
  const rows = await many(
    `WITH last_sent AS (
     ...
     SELECT n.id, n.email, n.subject, n.body, n.kind, n.severity, n.user_id,
      ...
      ORDER BY n.at LIMIT $1`, [limit * 5]);
  if (!rows.length) return { sent: 0, failed: 0 };

  /* Les abonnements décident enfin de TOUT ce qu'ils promettent.
     La table de 019 offre quatre réglages ; la remise n'en lisait que
     deux — la nature et la gravité — et ignorait la portée et la cadence
     par abonnement (O-1, docs/32 → docs/36 C-04). La règle du comité
     tient : un abonnement règle ce qui SORT, jamais ce qu'on peut venir
     chercher — le centre continue de tout recevoir, et seule la remise
     se laisse filtrer. Sans aucun abonnement, la préférence globale du
     compte gouverne, comme avant : la finesse s'ajoute, elle ne devient
  ...
  for (const [entity, list] of Object.entries(byEntity)) {
    ...
    if (!sql) continue;
    const found = await many(sql, [[...new Set(list.map((r) => String(r.entity_id)))]]);
  ...
  let sent = 0, failed = 0;
  for (const m of rows) {
    /* Read wider than the batch, because what a subscription or a cadence
       holds back stays queued at the head of the file and must not starve
       what may leave; the batch itself stays `limit`. */
