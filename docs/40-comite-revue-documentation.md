# 40 · Comité de revue documentaire — quatre sièges, 71 constats, trois défauts produit

> **Note de report, 23/09/2026 (`docs/36`, ligne C-04).** Ce document est
> le procès-verbal, conservé tel quel, du comité du **31/08/2026**. Il a
> siégé sur la branche `claude/project-analysis-db-schema-yph5ho`, qui
> n'a jamais été fusionnée, et il a revu les textes rédigés contre la
> **version 5.3.0**. Ces textes portaient alors les numéros 29 à 32 ; ils
> sont reportés ici sous les numéros **37 à 40**, parce que `main` avait
> entre-temps donné les numéros 29 à 36 à d'autres documents. Seuls les
> renvois internes ont été renumérotés.
>
> Les manuels ([`38`](38-user-manual.md), [`39`](39-manuel-utilisateur.md))
> et la référence technique ([`37`](37-technical-reference.md)) ont été
> **mis à jour pour 5.17.0 le 23/09** par la ligne C-04 de `docs/36`.
> Les corrections produit que ce comité décrit ont été faites sur la
> branche non fusionnée. P-01 est arrivé sur `main` par un autre chemin
> (5.9.1 et 5.10.0). P-02, P-03, O-1 à O-7 et la migration 027 de
> cette branche **n'étaient pas dans la version 5.17.0** : la moitié
> produit de C-04 les a relivrés le 23/09 sur l'arbre qui porte ce
> document (la migration 027 y devient **053**, numéro libre), et ils
> ont été rejoués ce jour-là. Les défauts qui restent, trouvés en
> rejouant les manuels, sont listés au §10 de la référence technique.
> Les chiffres ci-dessous (375 et 381 tests, neuf portes, 21 écrans ×
> 4 rôles) sont ceux du 31/08.

**Date : 31/08/2026 · Périmètre : la documentation courante** — le
README, le CHANGELOG, CONTRIBUTING, la référence technique
([`37`](37-technical-reference.md)) et les deux manuels utilisateur
([`38`](38-user-manual.md) EN, [`39`](39-manuel-utilisateur.md) FR).
...
|---|---|---|
| **Exactitude** | Chaque affirmation vérifiable de `37` est-elle vraie dans le code ? | 11 (≈150 affirmations contrôlées) |
| **Fidélité du manuel** | Chaque geste décrit dans `38` correspond-il à un écran, un bouton, un comportement réels ? | 23 (≈120 contrôlées) |
| **Langue française** | Le vocabulaire de `39` est-il celui du dictionnaire du produit (`web/src/lib/i18n.js`) ? | 27 |
...
Le siège de fidélité a relu `38` bouton par bouton. Au-delà des trois
...
Le siège de langue a confronté `39` au dictionnaire embarqué. Le
