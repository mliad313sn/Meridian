...
- **The JSON book carries only active sites, programmes and people.** A row
  pointing at an inactive one is refused by the import, and the refusal
  names it (NEW-18 in [`docs/36`](docs/36-convergence.md)). Everything
  else the export writes, the meeting register included, comes back field
...
server/          src/env.js        .env, the data directory, the one version number
                 ...
                 migrations/       ordered SQL (001–061, 65 tables, applied at boot)
...
| [`docs/37-technical-reference.md`](docs/37-technical-reference.md) | **Technical reference** (EN) — project description, the module map, the 65-table database schema, every functionality, the API surface, the 21 screens, deployment, and the known limits, each with its `docs/36` line |
