...
export const Engine = {
  ...
  /* O-7 (docs/32 → docs/36 C-04) — the same five-day rule, applied to
     the edges BETWEEN projects. The integrated master schedule drew these
     links and never checked them, so a feeding project could slide under
     its dependent with nothing said. ADDITIVE: depBreaches above and
     every number this module already produced are untouched. A stage is
     bounded by its activities' extremes, because a cross_dep names a
     stage (index), not an activity. */
  ...
  /** V-09 — what a person is actually available for. `availability` is
      ...
      string is directory information for the planner's eye (O-4). */
  effectiveFte(person) {
    if (!person) return 0;
