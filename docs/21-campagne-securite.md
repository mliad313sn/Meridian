...
| **S-11** | **Identifiants de base par défaut, silencieux.** Le paquet livrait `postgres:postgres` — l'identifiant le plus deviné du monde, et superutilisateur du cluster entier, pas seulement de cette base. Un défaut silencieux est un défaut que personne ne corrige. | **oui**, config constatée | **Résolu à la racine, 30/08** : le paquet ne livre plus aucun identifiant. L'installateur crée un rôle applicatif (`meridian`, propriétaire de sa seule base) avec un mot de passe tiré au hasard que personne n'a à choisir ni à retenir, et l'écrit dans une configuration que l'ACL durcie protège (S-09). Le démarrage garde l'avertissement pour les installations antérieures. |
